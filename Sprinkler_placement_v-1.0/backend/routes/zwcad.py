...
def zwcad_scenarios(req: ZWCADScenarioRequest):
    ...
    for sid in requested_ids:
        ...
        if f is None:
            continue
        deduped, culled, alpha_used, gama_used = f.result()
        # Format: (sid (heads) (outside) (alpha gama))
        ...
        # (alpha gama): how many row/column ends fired the alpha (stretch)
        #               / gama (new head + squeeze) formulas — the plugin
        #               prints these in the ZWCAD terminal. Older plugins
        #               read only the first elements and ignore the rest.
        print(f"[scenario {sid}] use alpha formula x{alpha_used}, "
              f"use gama formula x{gama_used}")
        payload_chunks.append(
            f"({sid} {_points_to_lisp(deduped)} {_outside_to_lisp(culled)} "
            f"({alpha_used} {gama_used}))"
        )
