...
from config import (
    ...
    WALL_CLEARANCE_MIN, MIN_HEAD_SEPARATION,
...
def generate_zone_sprinklers(
    ...
    if clearance >= WALL_CLEARANCE_MIN and outside_pts:
        ...
        for gx, gy in outside_pts:
            ...
            if best is None:
                continue
            _, hx, hy = best
            # User spec: eligible pulls ALWAYS fire; the only spacing guard
            # is MIN_HEAD_SEPARATION (config — the fill-density dial:
            # 800 = fill every gap, ~1500 = only real gaps).
            if spacing_hash.any_within(hx, hy, MIN_HEAD_SEPARATION):
                continue
    ...
    if clearance >= WALL_CLEARANCE_MIN:
        ...
        def _try_wall_head(px, py):
            ...
            if min_dist_to_segs(px, py, floor_segs_cached) < float(in_cov) - TOLERANCE:
                return
            # Spacing guard: MIN_HEAD_SEPARATION (config) only.
            if spacing_hash.any_within(px, py, MIN_HEAD_SEPARATION):
                return
...
def generate_zone_sprinklers_oriented(
    ...
    """
    # User-facing rule: align with the longest wall.
    angle = find_longest_edge_angle(floor_poly)
...
def run_scenario_for_floors(
    ...
    `tilted` controls whether per-room rotation is applied:
      * False (default) — axis-aligned placement only. Identical to the
        original (pre-rotation) behaviour: every returned point carries
        rotation 0.0, blocks are inserted upright.
      * True  — detect each room's longest-edge angle and rotate the
        placement grid to match. Returned rotation is the room's angle
        so the calling plugin can rotate inserted blocks accordingly.
    ...
    for fp in floor_polys:
        if tilted:
            ...
            # Straight: no rotation, exactly the original axis-aligned flow.
            result = generate_zone_sprinklers(
                floor_poly      = fp,
                excl_polys      = [],
                wall_segs       = [],
