...
# ── Obstacle test (v2: obstacles are ENFORCED — v1 ignored them) ──

def blocked_by_obstacle(px, py, obs_polys, obs_segs, obs_min_offset) -> bool:
    """
    True if (px, py) must be rejected because of an obstacle: either it
    lies INSIDE an obstacle polygon (a column, equipment pad, shaft) or
    it sits closer than `obs_min_offset` to any obstacle edge.

    v1 built `obs_segs` and then never queried it — heads landed inside
    columns. v2 calls this at every placement gate (grid, pull, wall row,
    Phase-4 add) so no head is ever placed in or hugging an obstacle.
    """
    for ob in obs_polys:
        if point_in_poly(px, py, ob):
            return True
    if obs_segs and obs_min_offset > 0:
        if min_dist_to_segs(px, py, obs_segs) < float(obs_min_offset) - TOLERANCE:
            return True
    return False
...
def _stretch_runs_to_walls(points, x_lines, y_lines, floor_poly, coverage_radius,
                           obs_polys=(), obs_segs=(), obs_min_offset=0.0):
    """
    ...
    for _sweep in range(3):
        ...
        def _try_add(ax, fixed, land):
            ...
            if min_dist_to_segs(cand[0], cand[1], wall_segs) < min_wall - TOLERANCE:
                return False
            # v2: an add may not land in or hugging an obstacle.
            if blocked_by_obstacle(cand[0], cand[1], obs_polys, obs_segs, obs_min_offset):
                return False
            for q in pts:
...
def generate_zone_sprinklers(
    ...
    for gy in reversed(y_lines):
        for gx in x_lines:
            if point_in_poly(gx, gy, floor_poly):
                # v2 obstacle rule: a grid point inside an obstacle (or
                # hugging its edge) is dropped outright. Pulling it toward
                # the boundary wouldn't help — the obstacle is interior —
                # so it doesn't go to outside_pts either.
                if blocked_by_obstacle(gx, gy, obs_polys, obs_segs, obs_min_offset):
                    rejected += 1
                    continue
    ...
    if clearance >= WALL_CLEARANCE_MIN and outside_pts:
        ...
        for gx, gy in outside_pts:
            ...
            if best is None:
                continue
            _, hx, hy = best
            # v2: a pull may not land in or hugging an obstacle.
            if blocked_by_obstacle(hx, hy, obs_polys, obs_segs, obs_min_offset):
                continue
    ...
    if clearance >= WALL_CLEARANCE_MIN and not GRID_ANCHOR_TOP_LEFT:
        ...
        def _try_wall_head(px, py):
            ...
            if min_dist_to_segs(px, py, floor_segs_cached) < float(in_cov) - TOLERANCE:
                return
            if blocked_by_obstacle(px, py, obs_polys, obs_segs, obs_min_offset):
                return
    ...
    points, alpha_used, gama_used = _stretch_runs_to_walls(
        ...
        obs_polys=obs_polys, obs_segs=obs_segs, obs_min_offset=obs_min_offset,
    )
