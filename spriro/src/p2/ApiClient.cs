...
namespace Spriro.P2
{
    ...
    {
        // v2 backend (the URL is resolved from spriro.config.json). Both
        // /api/zwcad/scenarios and /api/zwcad/auto live on the v2 server.
        public static readonly string BaseUrl = global::Spriro.SpriroConfig.P2BaseUrl;
        ...
        // v2 universal one-shot: rooms (+ labels + obstacles) in → one
        // compliant, minimal, verified layout + validation report out.
        // 10-min timeout — the pipeline (classify→place→verify→fix→
        // minimise→GA) over many rooms can run long.
        public static AutoResult PostZwcadAuto(AutoRequest req)
        {
            string lisp = PostJsonExpectText(
                BaseUrl + "/api/zwcad/auto",
                req,
                timeoutMs: 600000);
            return LispParser.ParseAuto(lisp);
        }
