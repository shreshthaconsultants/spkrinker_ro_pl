...
namespace Spriro.P2
{
    ...
    public class Commands
    {
        ...
        // v2 universal one-shot. Alias SPK2.
        [CommandMethod("SPK2")]
        public void Spk2() => AutoSprinkler2();

        // ── v2 UNIVERSAL command ──────────────────────────────────────
        //   1. Polyline layer (default layerx) → rooms.
        //   2. Obstacle layer (optional).
        //   3. Straight/Tilted.
        //   No scenario pick — the backend classifies each room's hazard
        //   from its TEXT label, places, verifies, self-fixes, minimises
        //   heads, and returns ONE compliant layout + a validation report
        //   (echoed to this terminal). Old heads auto-erase first.
        [CommandMethod("AUTOSPRINKLER2")]
        public void AutoSprinkler2()
        {
            ...
            var roomLayerOpts = new PromptStringOptions(
                "\n[v2] Enter polyline layer name <layerx>: ") { AllowSpaces = true };
            ...
            string layerName = string.IsNullOrWhiteSpace(roomLayerRes.StringResult)
                ? "layerx" : roomLayerRes.StringResult.Trim();
            ...
            var obsLayerOpts = new PromptStringOptions(
                "\nEnter obstacle layer name (Enter to skip) : ") { AllowSpaces = true };
            var obsLayerRes = ed.GetString(obsLayerOpts);
            string obstacleLayer =
                (obsLayerRes.Status == PromptStatus.OK
                 && !string.IsNullOrWhiteSpace(obsLayerRes.StringResult))
                    ? obsLayerRes.StringResult.Trim() : null;
            ...
            var request = new AutoRequest
            {
                ...
                default_hazard = "ordinary_2",
                conservative   = false,
                tilted         = tilted,
                minimise       = false,   // keep dense clean grid; circles
                                          // stay overlapping (no raggedness)
                enable_ga      = true,
            };
            ...
            try
            {
                result = ProgressDialog.RunWithPolling(
                    "Universal placement (classify + verify + fix)",
                    () => ApiClient.PostZwcadAuto(request),
            ...
            if (result.Heads.Count == 0)
            {
                ed.WriteMessage("\n[v2] Backend returned 0 heads.");
                return;
            }

            // Place (auto-erases any previous heads first).
            var drawn = new List<ObjectId>();
            var pts = new ScenarioPoints();
            pts.Inside.AddRange(result.Heads);
            try
            {
                int oldErased = PlaceBlocks(doc, pts, drawn);
                if (oldErased > 0)
                    ed.WriteMessage("\nAuto-erased {0} old head(s).", oldErased);
            }
            catch (Exception ex)
            {
            ...
            // Echo the validation report to the terminal.
            ed.WriteMessage("\n------------------------------");
            foreach (var line in result.ReportLines)
                ed.WriteMessage("\n{0}", line);
            ed.WriteMessage("\n------------------------------");
            ed.WriteMessage(
                "\n[v2] {0} heads placed. {1}{2}",
                result.TotalHeads,
                result.Passed ? "ALL ROOMS PASS." : "SOME ROOMS NEED REVIEW.",
                result.FlaggedRooms > 0
                    ? string.Format(" {0} room(s) flagged.", result.FlaggedRooms) : "");

            // Draw the NFPA-13 validation stamp next to the layout.
            try { DrawAutoStamp(doc, result, drawn); }
            catch (Exception ex)
            { ed.WriteMessage("\nStamp draw failed: {0}", ex.Message); }
        }

        // Draw an NFPA-13 validation stamp (text box) to the right of the
        // placed heads: sprinkler count, coverage %, and a green ✓ VALID /
        // red ✗ REVIEW mark. Tracked + on the INFO layer, so the next run's
        // auto-erase clears it.
        private static void DrawAutoStamp(
            Document doc, AutoResult result, List<ObjectId> drawn)
        {
            if (result.Heads.Count == 0) return;

            double maxX = double.MinValue, maxY = double.MinValue;
            foreach (var p in result.Heads)
            {
                if (p[0] > maxX) maxX = p[0];
                if (p[1] > maxY) maxY = p[1];
            }

            bool valid = result.Passed;
            // \U+2713 = ✓ tick, \U+2717 = ✗ cross. Green (ACI 3) / red (ACI 1).
            string mark = valid
                ? "{\\C3;\\U+2713 NFPA-13 VALID}"
                : "{\\C1;\\U+2717 NFPA-13 REVIEW}";

            string[] lines =
            {
                "{\\C40;— SPRINKLER VALIDATION —}",
                "",
                "{\\C40;sprinklers :} {\\C7;" + result.TotalHeads + "}",
                "{\\C40;coverage   :} {\\C7;" + result.Coverage.ToString("F1") + " %}",
                "{\\C40;rooms flag :} {\\C7;" + result.FlaggedRooms + "}",
                "",
                mark,
                "",
                "{\\H0.85x;\\C251;Powered by CADELI \\U+00B7 "
        ...
        // Walk model space and return every TEXT / MTEXT as [x, y, "text"]
        // for the v2 backend's hazard classifier. Position is the entity's
        // insertion point; the backend assigns each label to whichever room
        // polygon contains it, so labels outside any room are harmless.
        private static List<object[]> CollectLabels(Database db)
        {
            ...
            using (var tr = db.TransactionManager.StartTransaction())
            {
                ...
                foreach (ObjectId oid in ms)
                {
                    ...
                    if (ent is DBText t)
                    {
                        if (!string.IsNullOrWhiteSpace(t.TextString))
                            labels.Add(new object[]
                                { t.Position.X, t.Position.Y, t.TextString });
                    }
                    else if (ent is MText m)
                    {
                        // MText.Text strips formatting codes; Contents keeps
                        // them. Text is what we want for keyword matching.
                        string s = m.Text;
                        if (!string.IsNullOrWhiteSpace(s))
                            labels.Add(new object[]
                                { m.Location.X, m.Location.Y, s });
                    }
                }
                tr.Commit();
            }
